...
class LogicaCaja(FachadaCajaDeSeguridad):
    ...
    def editar_clave(self, id, nombre, clave, pista):
        if not all([id, nombre, clave, pista]):
    ...
    def crear_login(self, nombre, email, usuario, url, notas, id_claveFavorita):
        if not all([nombre, email, usuario, url, notas]):
            raise ValueError("Todos los campos deben tener un valor.")

        return ElementoRepositorio().guardar_Login_elemento(nombre, email, usuario,  url, notas, id_claveFavorita)
